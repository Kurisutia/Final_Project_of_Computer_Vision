...
def test(args):
    ...
    with torch.no_grad():
        for i, (image_path,image, img_name, folder, ori_size) in enumerate(vid_seq.data):
            ...
            orig = cv2.resize(orig,(1537,769))
            res= decoded.astype(np.uint8)
            result=orig*0.5+res*0.5
            imageio.imwrite(res_path, result.astype(np.uint8))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--img_path",nargs="?",type=str,
            default="./frame",help="Path_to_Frame",)

    parser.add_argument("--output_path",nargs="?",type=str,
            default="./out",help="Path_to_Save",)
